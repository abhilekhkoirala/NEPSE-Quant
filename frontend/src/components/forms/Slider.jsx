...
// so a rainbow of per-slider colors would just be noise.
function Slider({ label, min, max, step = 1, value, onChange, format = (v) => v, color = K.accent }) {
  return (
    <div style={{ marginBottom: SP.lg }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: SP.xs }}>
        <span style={{ fontSize: 13, color: K.textSecondary }}>{label}</span>
